...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function sendAudioToMake() {
        ...
        xhr.open('POST', 'https://hook.us2.make.com/q3o2vg1umjndy5nmvxp5rrlc939oy1p9', true);
        
        // Content-Type을 multipart/form-data로 변경
        const formData = new FormData();
        formData.append('audio', audioBlob, 'recording.wav');
        
        xhr.onload = function() {
            console.log("응답 받음:", xhr.status, xhr.responseText); // 디버깅 로그 추가
            if (xhr.status === 200) {
                try {
                    ...
                    console.error("JSON 파싱 에러:", error); // 디버깅 로그 추가
                    recordingStatus.textContent = '응답 처리 중 오류가 발생했습니다.';
                }
            } else {
                console.error("HTTP 에러:", xhr.status); // 디버깅 로그 추가
                recordingStatus.textContent = `오류가 발생했습니다. 상태 코드: ${xhr.status}`;
            }
        };
        
        xhr.onerror = function(error) {
            console.error("네트워크 에러:", error); // 디버깅 로그 추가
