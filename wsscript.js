...
const processFeedback = (rawText) => {
    ...
    for (const line of lines) {
        if (line.startsWith('별점:')) {
            // 숫자만 추출
            const ratingNum = line.substring(3).trim();
            // 숫자를 별표로 변환
...
const displayFeedback = (data) => {
    ...
    if (data && data.feedback) {
        feedbackContent = `
            <p><strong>별점:</strong> ${data.feedback.rating}</p>
            <p><strong>피드백:</strong> ${data.feedback.text}</p>
        `;
    } else {
...
fetch('API_ENDPOINT')
    ...
    .catch(error => {
        ...
        document.getElementById('feedback').innerHTML = '<p class="error">피드백을 받아오는 데 문제가 발생했습니다.</p>';
    });
